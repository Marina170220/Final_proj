...
def test_delete_book_from_wishlist(api_client: BookApi, wish_book_id):
    ...
    with allure.step("Проверить, что количество книг в избранном уменьшилось на 1"):
...
def test_delete_book_with_invalid_id(api_client: BookApi, wish_book_id):
    api_client.add_book_to_wishlist(wish_book_id)             
    wishlist_before = api_client.get_wishlist()
    api_client.delete_book_from_wishlist("Fy1258//")
    wishlist_after = api_client.get_wishlist()
    with allure.step("Проверить, что статус-код ответа 400 'Bad request'"):
        assert api_client == 400
    with allure.step("Проверить, что в избранном осталась одна книга"):
        assert len(wishlist_after) == 1
    with allure.step("Проверить, что количество книг в избранном не изменилось"):
